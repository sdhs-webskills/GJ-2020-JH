	...
	<section class="regPage rel">
		...
		<div class="cover rel">
			...
			<form action="/par/modify_action" method="post" enctype="multipart/form-data" class="gogo">
				...
				<div class="box rel">
					<p>축제명</p>
					<input type="text" name="names" placeholder="Write names..." value="<?= $list->nm ?>">
				</div>
				<div class="box flex cuts rel">
					<div>
						<p>지역</p>
						<input type="text" name="area" placeholder="Write your area..." value="<?= $list->area ?>">
					</div>
					<div>
						<p>기간</p>
						<input type="text" name="dt" placeholder="Write your Date(yyyy-MM-dd ~ yyyy-MM-dd)" value="<?= $list->dt ?>">
					</div>
				</div>
				<div class="box  rel">
					<p>장소</p>
					<input class="loca" type="text" name="location" placeholder="Write your location..." value="<?= $list->location ?>">
				</div>
				<div class="box rel">
					<p>축제 사진</p>
					<div class="flex rel">
			...
			<div class="img-cover rel">
				<?php for ($i = 0; $i < $len; $i++) { if($len != 0) { ?>
					<div class="img-box rel">
						<img src="/img/<?= $arr->sn ?>/<?= $i ?>" alt="<?= $arr->sn ?>/<?= $i ?>">
						<input type="checkbox" name="chk" id="chk<?= $i ?>"> <p><?= $arr->arr[$i] ?></p>
					</div>
				<?php } } ?>
			</div>
			<p>* 저장버튼을 누르면 체크한 이미지는 삭제된 상태로 저장 됩니다.</p>
